...
import { LiveCard } from "@/components/pump/live-card";
import { usePumpStore } from "@/lib/pump/store";
import type { Role } from "@/lib/pump/types";
import { fmtPct, fmtPrice } from "@/lib/pump/format";

export function Dashboard({ userName, userRole, onLogout }: { userName: string; userRole: Role; onLogout: () => void }) {
    ...
    <div className="flex min-h-screen flex-col bg-zinc-950 text-zinc-100">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-6 sm:px-6">
        ...
        <section className="mt-4">
          <EngineControls role={userRole} />
        </section>

        {/* watchlist manual */}
        <section className="mt-4">
          <WatchlistCard role={userRole} />
        </section>

        {/* trading real opt-in + telegram */}
        <section className="mt-4">
          <LiveCard role={userRole} />
        </section>
